...
const { data: profile, pending: loading } = await useFetch<User>('/api/client-api/v1/profile', { default: null, server: false })
...
function validate(state: any): FormError[] {
  const errors = []
  if (!state.fullName) errors.push({ path: 'name', message: 'Please enter your name.' })
    ...
    <UForm :state="state" :validate="validate" :validate-on="['submit']" @submit="onSubmit">
      <UDashboardSection title="Profile"
        ...
        <UFormGroup name="name" label="Name" description="Will appear on receipts, invoices, and other communication."
          ...
          <UInput v-model="state.fullName" autocomplete="off" icon="i-heroicons-user" size="md" :disabled="true" />
        </UFormGroup>
        ...
        <UFormGroup name="username" label="Username"
          ...
          <UInput v-model="state.username" type="username" autocomplete="off" size="md" input-class="ps-[77px]"
            :disabled="true">
            <template #leading>
              <span class="text-gray-500 dark:text-gray-400 text-sm">gmky.dev/</span>
            </template>
          </UInput>
        </UFormGroup>
        ...
        <UFormGroup name="avatar" label="Avatar" class="grid grid-cols-2 gap-2" help="JPG, GIF or PNG. 1MB Max."
          ...
          <UAvatar :src="state.avatar" :alt="state.fullName" size="lg" />

          <UButton label="Choose" color="white" size="md" @click="onFileClick" :disabled="true" />
        ...
        <UFormGroup name="bio" label="Bio" description="Brief description for your profile. URLs are hyperlinked."
          class="grid grid-cols-2 gap-2" :ui="{ container: '' }">
          <UTextarea v-model="state.bio" :rows="5" autoresize size="md" :disabled="true" />
        </UFormGroup>
